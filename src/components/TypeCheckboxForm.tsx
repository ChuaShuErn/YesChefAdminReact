            ...
            <FormGroup>
                {
                    options.map((label, index) => {
                        return <FormControlLabel key={index} label={label} checked={selectedOptions.indexOf(label) > -1} 
                        control={<Checkbox />} onChange={(e, checked) => setSelectedOptions(
                            (prevState: string[]) => checked ? prevState.concat(label) : prevState.filter((selectedOption: string) => 
